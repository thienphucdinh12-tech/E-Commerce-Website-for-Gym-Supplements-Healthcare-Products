    ...
    public List<Product> getFavoriteProducts(String username) throws Exception {
        ...
        try {
            ...
            if (conn != null) {
                ...
                // Bao gồm discount_price và discount_percent để hiển thị giá giảm
                String sql = "SELECT p.product_id, p.name, p.description, p.price, " +
                             "       p.discount_price, p.discount_percent, " +
                             "       p.is_flash_sale, p.sold_count, " +
                             "       p.image_url, p.stock_quantity " +
                             "FROM Favorites f " +
                             "JOIN ProductsWithStock p ON f.product_id = p.product_id " +
                ...
                while (rs.next()) {
                    ...
                    // Giá giảm (null nếu không có)
                    double discRaw = rs.getDouble("discount_price");
                    Double discountPrice   = rs.wasNull() ? null : discRaw;
                    int    discountPercent = rs.getInt("discount_percent");
                    boolean flashSale     = rs.getBoolean("is_flash_sale");
                    int     soldCount     = rs.getInt("sold_count");

                    Product p = new Product(id, name, description, price,
                                            discountPrice, discountPercent,
                                            flashSale, soldCount, quantity, imageUrl);
                    list.add(p);
                }
            }
        } catch (Exception e) {
