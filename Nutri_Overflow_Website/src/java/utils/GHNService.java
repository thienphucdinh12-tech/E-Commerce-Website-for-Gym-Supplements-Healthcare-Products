...
public class GHNService {
    ...
    // IMPORTANT: Replace with actual Sandbox/Production keys before deploying
    ...
    private static String sendRequest(String endpoint, String method, JsonObject payload) throws Exception {
        ...
        // Only send ShopId for endpoints that are not master-data
    ...
    public static JsonArray getProvinces() throws Exception {
        String response = sendRequest("/master-data/province", "GET", null);
        JsonObject json = gson.fromJson(response, JsonObject.class);
        return json.getAsJsonArray("data");
    }

    public static JsonArray getDistricts(int provinceId) throws Exception {
        JsonObject payload = new JsonObject();
        payload.addProperty("province_id", provinceId);
        String response = sendRequest("/master-data/district", "POST", payload);
        JsonObject json = gson.fromJson(response, JsonObject.class);
        return json.getAsJsonArray("data");
    }

    public static JsonArray getWards(int districtId) throws Exception {
        JsonObject payload = new JsonObject();
        payload.addProperty("district_id", districtId);
        String response = sendRequest("/master-data/ward?district_id=" + districtId, "POST", payload);
        JsonObject json = gson.fromJson(response, JsonObject.class);
        return json.getAsJsonArray("data");
    }

    public static double calculateFee(int toDistrictId, String toWardCode, int weightInGrams) throws Exception {
        JsonObject payload = new JsonObject();
        payload.addProperty("service_type_id", 2); // 2: Chuyển phát thương mại điện tử
        ...
        payload.addProperty("weight", weightInGrams);
        payload.addProperty("length", 10);
        payload.addProperty("width", 10);
        payload.addProperty("height", 10);

        String response = sendRequest("/v2/shipping-order/fee", "POST", payload);
        JsonObject json = gson.fromJson(response, JsonObject.class);
        return json.getAsJsonObject("data").get("total").getAsDouble();
    }
    ...
    public static boolean cancelOrder(String ghnOrderCode) throws Exception {
        JsonObject payload = new JsonObject();
        JsonArray codes = new JsonArray();
        codes.add(ghnOrderCode);
        payload.add("order_codes", codes);

        String response = sendRequest("/v2/switch-status/cancel", "POST", payload);
        JsonObject json = gson.fromJson(response, JsonObject.class);
    ...
    public static String createGHNOrderFromDB(int orderId) throws Exception {
        ...
        try (Connection conn = DBUtils.getConnection()) {
            ...
            // If it already has a valid GHN order code, return it
            ...
            // Also insert tracking if not present
            String trackSql = "INSERT INTO Order_Tracking (order_id, status, description, updated_at) " +
                              "VALUES (?, 'PROCESSING', N'Đơn giao hàng đã được tạo trên hệ thống Giao Hàng Nhanh.', GETDATE())";
            ...
            e.printStackTrace();
            System.err.println("GHN Create Order from DB error: " + e.getMessage());
            throw e;
        }
    }
}
