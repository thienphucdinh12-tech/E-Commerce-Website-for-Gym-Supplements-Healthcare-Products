...
public class VNPayOrderDAO {
    ...
    public boolean updatePaymentResult(String txnRef, String paymentStatus,
        ...
        String sql = "UPDATE Orders " +
                     ...
                     "                               ELSE status END " +
                     "WHERE txn_ref = ? AND ISNULL(payment_status, 'UNPAID') <> ?";
        try (Connection conn = DBUtils.getConnection();
             PreparedStatement ptm = conn.prepareStatement(sql)) {
            ...
            ptm.setString(8, paymentStatus);
            
            boolean updated = ptm.executeUpdate() > 0;
            
            if (updated && "PAID".equals(paymentStatus)) {
                int orderId = -1;
                String getIdSql = "SELECT order_id FROM Orders WHERE txn_ref = ?";
                try (PreparedStatement idPtm = conn.prepareStatement(getIdSql)) {
                    idPtm.setString(1, txnRef);
                    try (ResultSet rs = idPtm.executeQuery()) {
                        if (rs.next()) {
                ...
                if (orderId > 0) {
                    // Insert tracking record for processing
                    String trackSql = "INSERT INTO Order_Tracking (order_id, status, description, updated_at) " +
                                      "VALUES (?, 'PROCESSING', N'Thanh toán VNPay thành công. Shop đang chuẩn bị đơn hàng.', GETDATE())";
                    try (PreparedStatement trackPtm = conn.prepareStatement(trackSql)) {
                        trackPtm.setInt(1, orderId);
                        trackPtm.executeUpdate();
                    }

                    // Auto-create GHN shipping order upon successful VNPAY payment
                    try {
                        utils.GHNService.createGHNOrderFromDB(orderId);
                    } catch (Exception ghnEx) {
                        System.err.println("GHN Auto Creation Error on VNPAY payment for order #" + orderId + ": " + ghnEx.getMessage());
                    }
                }
            }
            return updated;
