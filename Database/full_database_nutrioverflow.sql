...
-- 4.0 ProductsWithStock View (Dung cho Website truy van luong ton kho)
...
INSERT INTO Favorites (user_id, product_id) VALUES (1, 1), (1, 2);
...
(1, 1, 5, N'Sữa uống rất ngon, tan nhanh, shop giao hàng hỏa tốc.'),

(2, 1, 4, N'Uống thấy người khỏe hơn hẳn, sẽ mua lại.');



-- 3.9 Orders (Đầy đủ khóa ngoại liên kết sang Customer có ID = 1 và thêm code)
...
VALUES (1, 2550000, 'SHIPPED', 1, 50000, 'NUTRIOVERFLOW50K', 'VNPAY', 'PAID', N'123 Đường ABC, Quận 1, TP.HCM');
...
INSERT INTO Articles (article_id, title, summary, content, image_url, author_username, author_name, status, is_published, created_at) VALUES 
  ...
  (1, N'Cẩm nang sử dụng Whey Protein đúng cách và an toàn', 
   ...
   N'<h4>1. Whey Protein là gì?</h4>

<p>Whey Protein là nguồn đạm chất lượng cao được chiết xuất từ sữa bò trong quá trình sản xuất phô mai. Nó chứa đầy đủ 9 loại axit amin thiết yếu (EAAs) mà cơ thể không tự tổng hợp được, với tốc độ hấp thụ siêu nhanh, giúp hỗ trợ đắc lực cho việc phục hồi và phát triển cơ bắp.</p>

<h4>2. Phân loại các loại Whey Protein phổ biến</h4>

<p>Trên thị trường hiện có 3 loại Whey Protein chính:</p>

<ul>

    <li><strong>Whey Protein Concentrate (WPC):</strong> Cuộc lọc đầu tiên, giữ lại nhiều chất dinh dưỡng tự nhiên của sữa nhưng chứa hàm lượng lactose và chất béo cao hơn. Phù hợp cho người mới bắt đầu hoặc muốn tiết kiệm chi phí.</li>

    <li><strong>Whey Protein Isolate (WPI):</strong> Đã trải qua quá trình lọc bỏ hầu hết lactose, fat và carbohydrate, đạt độ tinh khiết trên 90% protein. Rất tốt cho người siết cơ hoặc nhạy cảm với đường sữa lactose.</li>

    <li><strong>Whey Protein Hydrolyzed (WPH):</strong> Whey thủy phân cao cấp nhất, phân tách phân tử protein thành các mạch peptide ngắn giúp hấp thụ cực nhanh vào tế bào cơ mà không cần tốn nhiều thời gian tiêu hóa ở dạ dày.</li>

</ul>

<h4>3. Hướng dẫn sử dụng đúng liều lượng và thời điểm cần thiết</h4>

<p>Mỗi ngày người trưởng thành tập luyện nên dùng từ 1-2 muỗng để bổ sung protein hiệu quả. Hai thời điểm vàng để uống Whey là:</p>

<ul>

    <li><strong>Ngay sau khi tập:</strong> Đây là lúc cơ bắp cần protein nhất để sửa chữa và tái tạo các sợi cơ bị tổn thương sau buổi tập nặng.</li>

    <li><strong>Buổi sáng khi vừa ngủ dậy:</strong> Cơ thể đã trải qua 7-8 tiếng thiếu hụt dinh dưỡng khi ngủ, một ly Whey hấp thụ nhanh sẽ giúp ngăn chặn quá trình dị hóa cơ bắp.</li>

</ul>

<h4>4. Lưu ý quan trọng để uống Whey an toàn</h4>

<p>Không nên lạm dụng Whey Protein thay thế hoàn toàn cho bữa ăn chính. Hãy duy trì chế độ ăn uống đầy đủ từ thịt, cá, trứng và rau xanh. Nếu bạn bị dị ứng nặng với sữa bò, hãy cân nhắc sử dụng các loại protein thực vật (Plant Protein).</p>', 

   'image/articles/whey_guide.jpg', 'admin', N'Quản trị viên', 'APPROVED', 1, GETDATE()),

  (2, N'Chế độ ăn tăng cơ giảm mỡ (Recomp) cho người mới bắt đầu', 

   N'Body Recomposition là quá trình xây dựng cơ bắp đồng thời giảm mỡ thừa. Đây là mục tiêu hướng tới của rất nhiều Gymer.', 

   N'<h4>1. Nguyên lý hoạt động của Body Recomposition</h4>

<p>Body Recomposition (tái cấu trúc cơ thể) là quá trình giảm mỡ thừa đồng thời xây dựng cơ nạc cùng một lúc. Khác với phương pháp xả cơ (bulking) hoặc siết cơ (cutting) truyền thống, Recomposition tập trung vào việc cải thiện tỷ lệ cơ/mỡ mà không làm biến động quá lớn về cân nặng.</p>

<h4>2. Thiết lập chế độ dinh dưỡng tối ưu</h4>

<p>Để đạt được mục tiêu tăng cơ giảm mỡ đồng thời, bạn cần tuân thủ chế độ dinh dưỡng nghiêm ngặt:</p>

<ul>

    <li><strong>Thâm hụt calo nhẹ:</strong> Hãy duy trì mức calo nạp vào thấp hơn calo tiêu thụ khoảng 10-15% (khoảng 200-300 kcal). Thâm hụt quá sâu sẽ làm mất cơ bắp.</li>

    <li><strong>Nạp lượng protein dồi dào:</strong> Protein là chìa khóa duy nhất để xây dựng cơ. Bạn nên nạp từ 1.8g đến 2.2g protein trên mỗi kg trọng lượng cơ thể mỗi ngày.</li>

    <li><strong>Carbohydrate và Fat thông minh:</strong> Tập trung vào tinh bột hấp thu chậm (khoai lang, gạo lứt, yến mạch) and chất béo tốt (hạt hạnh nhân, bơ, dầu ô-liu).</li>

</ul>

<h4>3. Chế độ tập luyện kháng lực khoa học</h4>

<p>Tập luyện là kích thích bắt buộc để cơ thể giữ lại và phát triển cơ bắp trong trạng thái thâm hụt calo. Hãy tập tạ ít nhất 3-4 buổi/tuần, tập trung vào các bài tập đa khớp (Compound) như Squat, Deadlift, Bench Press, kết hợp với các buổi Cardio cường độ thấp (LISS) để tăng cường đốt mỡ.</p>', 

   'image/articles/recomp_diet.jpg', 'admin', N'Quản trị viên', 'APPROVED', 1, GETDATE()),

  (3, N'Eat Clean: Chế độ ăn lành mạnh cải thiện vóc dáng tối ưu', 

   N'Eat Clean không chỉ là một chế độ ăn kiêng, mà là lối sống lành mạnh giúp bạn thanh lọc cơ thể, giảm mỡ tự nhiên và duy trì năng lượng tích cực.', 

   N'<h4>1. Khái niệm Eat Clean là gì?</h4>

    <p>Eat Clean là phương pháp ăn uống ưu tiên sử dụng thực phẩm tươi sống, nguyên bản, chưa qua chế biến quá nhiều hoặc không chứa các chất phụ gia, chất bảo quản nhân tạo. Mục tiêu cốt lõi của Eat Clean là giúp cơ thể hấp thụ nguồn dinh dưỡng tinh khiết nhất.</p>

    <h4>2. Các nguyên tắc cơ bản khi ăn Eat Clean</h4>

    <p>Để bắt đầu Eat Clean hiệu quả, bạn cần ghi nhớ các nguyên tắc vàng sau:</p>

    <ul>

        <li><strong>Ăn thực phẩm nguyên bản:</strong> Chọn gạo lứt thay cho gạo trắng, chọn yến mạch nguyên cám, ăn trái cây tươi thay vì uống nước ép đóng chai.</li>

        <li><strong>Hạn chế tối đa đường tinh luyện:</strong> Tránh xa bánh kẹo ngọt, nước ngọt có ga. Hãy dùng mật ong hoặc đường cỏ ngọt tự nhiên để tạo ngọt nhẹ.</li>

        <li><strong>Tự nấu ăn tại nhà:</strong> Giúp bạn kiểm soát tốt lượng dầu mỡ, muối và gia vị nạp vào cơ thể.</li>

        <li><strong>Ăn nhiều rau xanh và trái cây:</strong> Đảm bảo cung cấp đủ lượng chất xơ, vitamin và khoáng chất tự nhiên giúp hệ tiêu hóa khỏe mạnh.</li>

    </ul>

    <h4>3. Lợi ích vượt trội cho sức khỏe và vóc dáng</h4>

    <p>Ăn Eat Clean đều đặn giúp bạn giảm mỡ thừa tự nhiên, làn da sáng khỏe mịn màng hơn nhờ được thanh lọc tốt. Đồng thời cải thiện hệ tim mạch, tăng mức năng lượng và giảm mệt mỏi trong các hoạt động hằng ngày.</p>', 

   'image/articles/eat_clean_guide.jpg', 'admin', N'Quản trị viên', 'APPROVED', 1, GETDATE());
